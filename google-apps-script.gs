...
// Set via: Project Settings -> Script Properties -> TELEGRAM_BOT_TOKEN
// (or run setTelegramBotToken() once from the editor with the new token)
function getTelegramBotToken() {
  return PropertiesService.getScriptProperties().getProperty("TELEGRAM_BOT_TOKEN");
}

function setTelegramBotToken() {
  const token = Browser.inputBox("Paste the new Telegram bot token:");
  if (token && token.indexOf("Cancel") !== 0) {
